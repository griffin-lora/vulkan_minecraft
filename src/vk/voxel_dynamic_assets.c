...
void transfer_voxel_region(const staging_t* staging, const voxel_region_render_info_t* render_info) {
...
void end_voxel_region(const staging_t* staging) {
    vmaDestroyBuffer(allocator, staging->buffer, staging->allocation);
}

void destroy_voxel_dynamic_assets(void) {
    for (size_t i = 0; i < NUM_QUEUED_VOXEL_DYNAMIC_ASSSETS; i++) {
        const voxel_region_render_info_t* render_infos = voxel_region_render_info_arrays[i];
        const voxel_region_allocation_info_t* allocation_infos = voxel_region_allocation_info_arrays[i];

        for (size_t j = 0; j < NUM_VOXEL_REGIONS; j++) {
            vmaDestroyBuffer(allocator, render_infos[j].vertex_buffer, allocation_infos[j].vertex_allocation);
        }
    }
}